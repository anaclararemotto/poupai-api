...
class CategoriaController {
  static async listarCategorias(req, res) {
    try {
      ...
      res
        .status(500)
        .send({ message: `${erro.message} - falha ao listar categorias` });
    }
  }
  ...
  static async listarCategoriaPorId(req, res) {
    try {
      ...
      res
        .status(500)
        .send({
          message: `${erro.message} - falha ao listar categoria por ID`,
        });
    }
  }
  ...
  static async listarCategoriasPorTipo(req, res) {
    try {
      ...
      res
        .status(500)
        .json({
          message: `Erro ao listar categorias por tipo: ${error.message}`,
        });
    }
  }
  ...
  static async cadastrarCategoria(req, res) {
    try {
      ...
      res
        .status(500)
        .send({ message: `${erro.message} - falha ao cadastrar categoria` });
    }
  }
